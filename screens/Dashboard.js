...
import { dummyData, COLORS, icons, images, FONTS, SIZES } from "../constants";
import { TextButton } from "../components";

const COUNTRIES_ITEM_SIZE = SIZES.width / 3;
const PLACES_ITEM_SIZE =
  Platform.OS === "ios" ? SIZES.width / 1.5 : SIZES.width / 1.2;
...
const Dashboard = ({ navigation }) => {
  ...
  const [placesScrollPosition, serPlacesScrollPosition] = useState(0);
    ...
    return (
      <Animated.FlatList
        ...
        onMomentumScrollEnd={(event) => {
          ...
          serPlacesScrollPosition(position);
        }}
        renderItem={({ item, index }) => {
